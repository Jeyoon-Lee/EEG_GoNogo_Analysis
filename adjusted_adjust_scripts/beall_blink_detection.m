...
%% Get Z-Scores Matrix
% [RANK-AWARE 수정] zmatrix는 IC×채널이므로 열 차원을 nchannels로.
% (pca rank 축소로 IC수<채널수일 때 채널 인덱스 경계 초과 방지. 판정 로직은 그대로.)
...
if length(blinks)>0                                            %
    ...
    eye_electrodes=[];                                         %
    % [RANK-AWARE 수정] 채널 순회이므로 nchannels 사용 (IC<채널 시 채널 누락 방지)
    for e=1:nchannels                                          %
