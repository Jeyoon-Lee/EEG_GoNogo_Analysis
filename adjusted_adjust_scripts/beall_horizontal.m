...
%% Get Z-Scores Matrix
% [RANK-AWARE 수정] 원본은 zmatrix=zeros(n,n)으로 IC수=채널수를 가정했다.
% pca로 rank 축소 시 IC수(n) < 채널수(nchannels)가 되면, 채널 인덱스(index1 등)로
% zmatrix 열에 접근할 때 경계를 초과한다. zmatrix는 실제로 IC×채널이므로
% 열 차원을 nchannels로, 채널 순회를 nchannels로 바로잡는다.
% (판정 로직/임계값은 그대로. 차원 불일치만 수정.)
zmatrix = zeros(n,nchannels);
for i=1:n % for each topography (IC)
    curr_vect = zeros(nchannels,1);
    %deep copy ICA matrix (채널 차원으로 순회)
    for h=1:nchannels
...
if length(horizontal) > 0 %if there are artifacts               % 
    ...
    eye_electrodes=[];                                          %
    % [RANK-AWARE 수정] 채널 순회이므로 n(IC수)이 아니라 nchannels 사용.
    % (IC<채널일 때 뒤쪽 채널이 누락되어 eye_electrodes가 불완전해지는 것 방지)
    for e=1:nchannels                                          %
